...
Because the AUC focuses on how well the model ranks positive instances above negative ones, it is generally more robust to imbalanceness in data distributions rather than metrics like accuracy or F1 score.
...
We also used a fixed \texttt{random\_state=0} (which is not actually an hyperparameter) to ensure reproducibility of results.
...
% boxplot explanation (mean of AUC scores across the 11 mixture probabilities, for each experiment. underline the fact of the MEAN)
